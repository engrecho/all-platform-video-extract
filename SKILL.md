...
通过复刻 greenvideo.cc 前端加密流程（AES-128-CBC + RSA-1024 PKCS#1 v1.5 长文本分段加密），调用其 `/api/video/cnSimpleExtract` 接口，解析视频链接并获取标题、封面、各清晰度下载直链。支持 1000+ 视频平台（抖音、快手、B 站、YouTube、TikTok、小红书、微博等）。

核心脚本 `scripts/greenvideo_extract.cjs` 已实现完整的加密复刻与接口调用，仅依赖 Node.js 内置模块（`crypto`、`fetch`），无需安装任何第三方包。

## Supported Platforms

greenvideo.cc 官网宣称支持 1000+ 平台。以下是首页明确列出的平台（带独立入口的为主要支持，列表中的为额外支持）：
...
- 用户提到 greenvideo.cc、cnSimpleExtract、GreenVideo、kedou@8989 等 greenvideo 协议关键字
- 用户在排查 greenvideo 加密失败（code=530）、想用 `--replay` 模式重放抓包 body
- 用户想理解 greenvideo 前端 AES+RSA 加密流程（此时引导读 `references/encryption_flow.md`）
...
```bash
node ~/.workbuddy/skills/greenvideo-extract/scripts/greenvideo_extract.cjs "<视频分享文本或URL>"
```

示例（抖音分享文本）：

```bash
node ~/.workbuddy/skills/greenvideo-extract/scripts/greenvideo_extract.cjs "3.58 复制打开抖音，看看【_馬冬的作品】出门在外身份是自己给的！！ # 文静小女生 # 馬... https://v.douyin.com/q3Xf96DFFCk/ Iic:/ m@Q.xF 03/03"
```

示例（B 站 URL）：

```bash
node ~/.workbuddy/skills/greenvideo-extract/scripts/greenvideo_extract.cjs "https://www.bilibili.com/video/BV1ypdgBCE9B/"
```
...
```bash
node ~/.workbuddy/skills/greenvideo-extract/scripts/greenvideo_extract.cjs --replay "<抓包拿到的加密 body 字符串>"
```
...
```bash
node ~/.workbuddy/skills/greenvideo-extract/scripts/greenvideo_extract.cjs
```
...
```bash
# 方式一：直接传分享文本或 URL
node ~/.workbuddy/skills/greenvideo-extract/scripts/download_videos.cjs \
...
# 方式三：从文件读（每行一个）
node ~/.workbuddy/skills/greenvideo-extract/scripts/download_videos.cjs urls.txt
```
...
/Users/jaylon/.workbuddy/binaries/node/versions/22.22.2/bin/node \
  ~/.workbuddy/skills/greenvideo-extract/scripts/greenvideo_extract.cjs \
  "<分享文本或URL>"
```
...
- **网络错误**：greenvideo.cc 偶尔不稳定，建议间隔几秒重试。
...
- **Cookie 自动获取**：脚本会先 `GET https://greenvideo.cc/` 拿 Set-Cookie，网站免登录
...
1. 自动访问 greenvideo.cc 获取 cookie
...
- greenvideo.cc 是公开免登录网站，脚本仅调用其公开接口，不涉及绕过付费/鉴权
...
- **源文件位置**：skill 源文件在项目目录 `ExtractVideoSkill/`，`~/.workbuddy/skills/greenvideo-extract/` 是指向它的软链接，编辑任一侧都生效
